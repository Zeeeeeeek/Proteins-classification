...
def run_query(query_string, file_name, merge_regions):
    ...
    preprocess_from_json(repeatsdb_get("query=" + q), merge_regions).to_csv(file_name + ".csv", index=False)
...
def run_kmer_count(input_file, k, output_file):
    ...
    multithread_kmer_count_df("csv/"+input_name, k).to_csv(output_name, index=False)
